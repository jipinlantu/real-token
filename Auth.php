...
class Auth
{
    ...
    public function __construct($options = [],$usertable='')
    {
        // var_dump($usertable);die;
     ...
     * @return Auth
     */
    public static function instance($options = [],$usertable='')
    {
 
    
        if (is_null(self::$instance)) {
           
            self::$instance = new static($options,$usertable);
        }
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            $user = Db::name( $this->usertable)->where("id",$user_id)->find(); 
            $user["group"] = Db::name( $this->usertable."_group")->where("id",$user["group_id"])->find(); 
            if (!$user) {
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        if (Db::name( $this->usertable)->where("username",$username)->find()) {
            $this->setError('Username already exist');
            return false;
        }
        if (Db::name( $this->usertable)->where("nickname",$username)->find()) {
            $this->setError('Nickname already exist');
            return false;
        }
        if ($email && Db::name( $this->usertable)->where("email",$email)->find()) {
            $this->setError('Email already exist');
            return false;
        }
        if ($mobile && Db::name( $this->usertable)->where("mobile",$mobile)->find()) {
        ...
        Db::startTrans();
        try {
            $user_id = Db::name( $this->usertable)->where("username",$username)->insertGetId($params);
// var_dump($user_id);die;
            $this->_user = Db::name( $this->usertable)->where("id",$user_id)->find();

            //设置Token
            $this->_token = Random::uuid();
            Token::set($this->_token, $user_id, $this->keeptime);
    ...
    public function login($account, $password)
    {
        ...
        //  User::get([$field => $account])
        $user = Db::name( $this->usertable)->where($field,$account)->find();
        ...
        // var_dump($user['password'],$this->getEncryptPassword($password, $user['salt']));die;
    ...
    public function direct($user_id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                //判断连续登录和最大连续登录
                if ($user["logintime"] < $this->unixtime('day')) {
                    $user['successions'] = $user['logintime'] <  $this->unixtime('day', -1) ? 1 : $user['successions'] + 1;
                ...
                //重置登录失败次数
                $user['loginfailure'] = 0;
                Db::name( $this->usertable)->where("id",$user_id)->update($user);
  ...
  /**
     ...
     */
    public  function unixtime($type = 'day', $offset = 0, $position = 'begin', $year = null, $month = null, $day = null, $hour = null, $minute = null)
    {
    ...
    public function getRuleList()
    {
        ...
        $this->rules = Db::name($this->usertable."_rule")->where('status', 'normal')->where('id', 'in', $rules)->field('id,pid,name,title,ismenu')->select();
        return $this->rules;
    }
    ...
    public function delete($user_id)
    {
        ...
        try {
            // 删除会员
            Db::name($this->usertable)->where('id',$user_id)->delete($user_id);
            // 删除会员指定的所有Token
            Token::clear($user_id);
